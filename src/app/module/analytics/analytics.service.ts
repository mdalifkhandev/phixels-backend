...
const getSourceCondition = (source?: string): FilterQuery<TAnalyticsEvent> => {
  ...
  if (key === 'social media') {
    return {
      $or: [
        ...
        { referrer: { $regex: '(facebook|instagram|linkedin|twitter|tiktok)', $options: 'i' } },
      ],
    };
  }
  ...
  if (key === 'direct') {
    return {
      $or: [{ referrer: { $in: [null, ''] } }, { referrer: { $exists: false } }],
    };
  }
...
const getEventsFromDB = async (params: TRangeParams & { eventType?: string; limit?: number }) => {
...
const getCampaignPerformanceFromDB = async (params: TRangeParams) => {
  ...
  const data = await AnalyticsEvent.aggregate([
    { $match: match },
    {
      $addFields: {
        campaignName: {
          $cond: [
            { $or: [{ $eq: ['$utmCampaign', null] }, { $eq: ['$utmCampaign', ''] }] },
            '(direct)',
            '$utmCampaign',
          ],
        },
  ...
  return data.map((item) => {
    ...
    const lastEventAt = item.lastEventAt ? new Date(item.lastEventAt) : new Date(0);
    const daysSinceLastEvent = (Date.now() - lastEventAt.getTime()) / (1000 * 60 * 60 * 24);
    const status = daysSinceLastEvent <= 7 ? 'Active' : daysSinceLastEvent <= 30 ? 'Paused' : 'Ended';
...
const getPlatformPerformanceFromDB = async (params: TRangeParams) => {
  ...
  const platforms = {
    google: { id: 'google', name: 'Google Ads', impressions: 0, clicks: 0, conversions: 0, lastSync: null as Date | null },
    facebook: { id: 'facebook', name: 'Meta Ads', impressions: 0, clicks: 0, conversions: 0, lastSync: null as Date | null },
    linkedin: { id: 'linkedin', name: 'LinkedIn Ads', impressions: 0, clicks: 0, conversions: 0, lastSync: null as Date | null },
  };

  const resolvePlatform = (event: any): keyof typeof platforms | null => {
    const source = `${event.utmSource || ''} ${event.referrer || ''}`.toLowerCase();
  ...
  return Object.values(platforms).map((platform) => ({
    ...platform,
    status: platform.impressions + platform.clicks + platform.conversions > 0 ? 'connected' : 'disconnected',
    spend: 0,
  }));
};
