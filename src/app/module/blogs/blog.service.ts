...
const updateBlog = async (id: string, payload: Partial<TBlog>) => {
    ...
    if (payload.isFeatured === true) {
        await Blog.updateMany({ _id: { $ne: id }, isFeatured: true }, { isFeatured: false, featuredOrder: null });
...
const updateFeatureStatus = async (id: string, payload: { isFeatured: boolean; featuredOrder?: number | null }) => {
